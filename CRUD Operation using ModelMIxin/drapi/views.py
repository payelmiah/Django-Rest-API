...
#class based view for list and create
class AiquestList_List_Create(GenericAPIView, ListModelMixin, CreateModelMixin):
...
class Aiquest_Retrieve_update_destroy(RetrieveModelMixin, GenericAPIView, UpdateModelMixin, DestroyModelMixin):
    queryset = Aiquest.objects.all()
    serializer_class = AiquestSerializer
    def get(self, request, *args, **kwargs):
